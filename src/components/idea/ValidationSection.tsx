...
export default function ValidationSection({
  ...
  return (
    <div
      className={`rounded-xl bg-white p-3.5 shadow-[0_1px_2px_rgba(0,0,0,0.03),0_0_0_1px_rgba(0,0,0,0.04)] transition-all duration-300 ${
        content ? "validation-reveal" : ""
      }`}
    >
      <div className="flex items-center gap-2">
        <span className="flex h-6 w-6 items-center justify-center rounded-md bg-zinc-50 text-[13px]">
          {icon}
        </span>
      ...
      {content ? (
        ...
        <div className="mt-2.5 space-y-1.5">
          ...
          <div className="h-3 w-3/5 animate-pulse rounded bg-zinc-50" />
        </div>
      )}
    </div>
  );
